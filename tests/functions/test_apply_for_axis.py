...
def test_apply_for_axis_4d_tensor() -> None:
    ...
    expected = torch.tensor([[[[1.0, 3.0], [3.0, 7.0]], [[5.0, 11.0], [7.0, 15.0]]]])
    np.testing.assert_allclose(result.numpy(), expected.numpy())
...
def test_apply_for_axis_different_dtypes() -> None:
    ...
    expected_double = torch.tensor([[2.0, 3.0], [4.0, 5.0]], dtype=torch.float64)
...
def test_apply_for_axis_middle_dimensions() -> None:
    ...
    expected = torch.tensor([[[4.0, 6.0], [4.0, 6.0]], [[12.0, 14.0], [12.0, 14.0]]])
    np.testing.assert_allclose(result.numpy(), expected.numpy())
...
def test_apply_for_axis_zero_dimension_axis() -> None:
    ...
    expected = torch.tensor([[1.0/3.0, 2.0/3.0], [3.0/7.0, 4.0/7.0], [5.0/11.0, 6.0/11.0]])
    np.testing.assert_allclose(result.numpy(), expected.numpy())
...
def test_apply_for_axis_complex_function() -> None:
    ...
    np.testing.assert_allclose(row_sums.numpy(), expected_sums.numpy(), atol=1e-6)
...
def test_apply_for_axis_broadcasting_behavior() -> None:
    ...
    expected = torch.tensor([[[11.0, 21.0], [12.0, 22.0]], 
                            [[13.0, 23.0], [14.0, 24.0]]])
    np.testing.assert_allclose(result.numpy(), expected.numpy())