...
def test_soft_topk_bottomk_3d_tensor() -> None:
    ...
    assert result.shape == x.shape
    np.testing.assert_allclose(result.sum(dim=2).numpy(), [[0.0], [0.0]], atol=1e-5)
...
def test_soft_topk_3d_tensor() -> None:
    ...
    assert result.shape == x.shape
    np.testing.assert_allclose(result.sum(dim=2).numpy(), [[1.0], [1.0]], atol=1e-5)
    assert (result >= 0).all()
...
def test_soft_topk_extreme_epsilon_values() -> None:
    ...
    assert abs(result_small.sum(dim=1).item() - 1.0) < 2e-5  # More lenient tolerance
