...
def test_mulsum_mean_mode() -> None:
    ...
    np.testing.assert_allclose(
        result_mean.numpy(), expected_mean.numpy(), rtol=1e-4, atol=1e-4
    )
    ...
    np.testing.assert_allclose(
        result_sum.numpy(), expected_sum.numpy(), rtol=1e-4, atol=1e-4
    )
...
def test_mulsum_high_dimensional() -> None:
    ...
    np.testing.assert_allclose(
        result_dim3.numpy(), expected_dim3.numpy(), rtol=1e-4, atol=1e-3
    )
...
def test_mulsum_complex_broadcasting() -> None:
    ...
    np.testing.assert_allclose(result1.numpy(), expected1.numpy(), rtol=1e-4, atol=1e-4)
...
def test_mulsum_performance_comparison() -> None:
    ...
    for size in [50, 100]:
        ...
        np.testing.assert_allclose(
            result_mulsum.numpy(), result_naive.numpy(), rtol=1e-4, atol=1e-4
        )
