...
def test_eigh_symmetric_matrix() -> None:
    ...
    np.testing.assert_allclose(torch.abs(v).numpy(), torch.abs(expected_v).numpy(), atol=1e-4)
...
def test_eigh_reconstruction() -> None:
    ...
    A = torch.tensor([[3.0, 1.0, 1.0], [1.0, 3.0, 1.0], [1.0, 1.0, 3.0]], dtype=torch.float32)
    w, v = QF.eigh(A)
...
def test_eigh_orthogonality_of_eigenvectors() -> None:
    ...
    np.testing.assert_allclose(orthogonality_check.numpy(), identity.numpy(), atol=1e-5)
